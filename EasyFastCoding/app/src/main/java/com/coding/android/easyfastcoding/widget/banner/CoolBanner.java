package com.coding.android.easyfastcoding.widget.banner;
...
import com.coding.android.easyfastcoding.widget.banner.transformer.AccordionPageTransformer;
import com.coding.android.easyfastcoding.widget.banner.transformer.AlphaPageTransformer;
import com.coding.android.easyfastcoding.widget.banner.transformer.CubePageTransformer;
import com.coding.android.easyfastcoding.widget.banner.transformer.DefaultPageTransformer;
import com.coding.android.easyfastcoding.widget.banner.transformer.DepthPageTransformer;
import com.coding.android.easyfastcoding.widget.banner.transformer.FadePageTransformer;
import com.coding.android.easyfastcoding.widget.banner.transformer.FlipPageTransformer;
import com.coding.android.easyfastcoding.widget.banner.transformer.RotatePageTransformer;
import com.coding.android.easyfastcoding.widget.banner.transformer.StackPageTransformer;
import com.coding.android.easyfastcoding.widget.banner.transformer.ZoomCenterPageTransformer;
import com.coding.android.easyfastcoding.widget.banner.transformer.ZoomFadePageTransformer;
import com.coding.android.easyfastcoding.widget.banner.transformer.ZoomPageTransformer;
import com.coding.android.easyfastcoding.widget.banner.transformer.ZoomStackPageTransformer;
 ...
 * 提供给外部的公用方法：
 * <p>
 * 1、setPageChangeDuration(int) : 设置页码切换的等待时间
 * <p>
 * 2、setTransitionEffect(TransitionEffect) : 设置页码切换的动画效果，参数为内部枚举类型
 * <p>
 * 3、setPageTransformer(PageTransformer) : 设置自定义的页码切换效果
 * <p>
 * 4、setViewsAndTips(List<View>,List<String>) : 设置切换的页码内容和提示类容
 * <p>
 * 5、setViews(List<View>) : 设置切换的页码内容
 * <p>
 * 6、addOnPageChangeListener(OnPageChangeListener) : 页面改变的监听
 * <p>
 * 7、startAutoPlay() : 开始自动轮播
 * <p>
 * 8、stopAutoPlay() : 停止自动轮播
 * <p>
 * 9、setCurrentItem() : 设置当前的页码数
 * <p>
 * Created by cy on 2015/11/30.
 */
public class CoolBanner extends RelativeLayout {

    private static final String TAG = CoolBanner.class.getSimpleName();
     ...
     * 指示点的图片资源
     */
    private int mPointDrawableResId = R.drawable.ef_selector_coolbanner_pointer;
    /**
     * 指示点的重心
     */
    ...
    private Handler mAutoPlayHandler = new Handler() {
        ...
        public void handleMessage(Message msg) {
            ...
            mAutoPlayHandler.sendEmptyMessageDelayed(WAIT_AUTO_PLAY, mAutoPlayInterval);
        }
    };

    public CoolBanner(Context context, AttributeSet attrs) {
        this(context, attrs, 0);
    }

    public CoolBanner(Context context, AttributeSet attrs, int defStyleAttr) {
    ...
    private void initDefaultAttrs(Context context) {
        mViewPager = new CoolViewPager(context);
    ...
    private void initCustomAttrs(Context context, AttributeSet attrs) {
        TypedArray typedArray = context.obtainStyledAttributes(attrs, R.styleable.CoolBanner);
    ...
    private void initCustomAttr(int attr, TypedArray typedArray) {
        if (attr == R.styleable.CoolBanner_banner_pointDrawable) {
            mPointDrawableResId = typedArray.getResourceId(attr, R.drawable.ef_selector_coolbanner_pointer);
        } else if (attr == R.styleable.CoolBanner_banner_pointContainerBackground) {
            mPointContainerBackgroundDrawable = typedArray.getDrawable(attr);
        } else if (attr == R.styleable.CoolBanner_banner_pointLeftRightMargin) {
            mPointLeftRightMargin = typedArray.getDimensionPixelSize(attr, mPointLeftRightMargin);
        } else if (attr == R.styleable.CoolBanner_banner_pointContainerLeftRightPadding) {
            mPointContainerLeftRightPadding = typedArray.getDimensionPixelSize(attr, mPointContainerLeftRightPadding);
        } else if (attr == R.styleable.CoolBanner_banner_pointTopBottomMargin) {
            mPointTopBottomMargin = typedArray.getDimensionPixelSize(attr, mPointTopBottomMargin);
        } else if (attr == R.styleable.CoolBanner_banner_pointGravity) {
            mPointGravity = typedArray.getInt(attr, mPointGravity);
        } else if (attr == R.styleable.CoolBanner_banner_pointAutoPlayAble) {
            mAutoPlayAble = typedArray.getBoolean(attr, mAutoPlayAble);
        } else if (attr == R.styleable.CoolBanner_banner_pointAutoPlayInterval) {
            mAutoPlayInterval = typedArray.getInteger(attr, mAutoPlayInterval);
        } else if (attr == R.styleable.CoolBanner_banner_pageChangeDuration) {
            mPageChangeDuration = typedArray.getInteger(attr, mPageChangeDuration);
        } else if (attr == R.styleable.CoolBanner_banner_transitionEffect) {
            int ordinal = typedArray.getInt(attr, TransitionEffect.Accordion.ordinal());
            setTransitionEffect(TransitionEffect.values()[ordinal]);
        } else if (attr == R.styleable.CoolBanner_banner_tipTextColor) {
            mTipTextColor = typedArray.getColor(attr, mTipTextColor);
        } else if (attr == R.styleable.CoolBanner_banner_tipTextSize) {
    ...
    private final class ChangePointListener extends CoolViewPager.SimpleOnPageChangeListener {
